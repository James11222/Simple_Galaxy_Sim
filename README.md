...
This simulation does not rely on derived physical models. This script is meant to demonstrate how one can simulate a galaxy in python. With careful consideration one can include more robust physical models than the ones used here. We use a characteristic constant rotational velocity curve devided by a Gaussian function centered on the Black Hole to mimic a typical velocity curve to mimic the effects of dark matter. We populate the galaxy with a central suppermassive black hole ~1000x more massive than the rest of the stars in the galaxy and the stars follow a decaying exponential distribution in their instantiation. Using standard Newtonian gravity we calculate the forces of gravity via a naive O(N^2) computation algorithm. This simulation is collisionless.
